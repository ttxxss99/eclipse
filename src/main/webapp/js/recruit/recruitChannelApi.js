...
layui.use('table', function(){
   ...
   table.render({
        ...
        ,cols: [[
            ...
            ,{field:'name', title:'渠道名称或地址', width:"25%"}
            ,{field:'addtime', title:'渠道上传时间', width:"25%"}
            ,{field:'mark', title:'备注', width:"25%"}
            ,{fixed: 'right', title:'操作', toolbar: '#barDemo', width:"25%"}
        ]]
        ,page:true
        //异步请求响应
        ,url: '/recruit/channel/channelListPage'
        ,request: {
    ...
    table.on('tool(layTable)', function(obj){
        ...
        if(obj.event === 'detail'){
            ...
            console.log(data.id);
            let url = '/recruit/channel/channelUpdate?id='+data.id;
            console.log(url);
            x_admin_show('编辑',url);
...
function member_del(id){
    let url = "/recruit/channel/channelDeleteAjax?id="+id;
