...
layui.use('table', function(){
   ...
   table.render({
        ...
        ,cols: [[
            ...
            ,{field:'sexStr', title:'性别', width:"6.3%"}
            ,{field:'qq', title:'QQ', width:"8.3%"}
            ,{field:'phone', title:'电话', width:"8.3%"}
            ,{field:'email', title:'邮箱', width:"8.3%"}
            ,{field:'age', title:'年龄', width:"6.3%"}
            ,{field:'currentaddress', title:'现住址', width:"8.3%"}
            ,{field:'applicanttime', title:'应聘时间', width:"8.3%"}
            ,{field:'specialty', title:'技术特长', width:"8.3%"}
            ,{field:'comments', title:'面试官评价', width:"8.3%"}
            ,{field:'mark', title:'备注', width:"8.3%"}
            ,{fixed: 'right', title:'操作', toolbar: '#barDemo', width:"12.7%"}
        ]]
        ,page:true
        //异步请求响应
        ,url: '/recruit/candidate/candidateListPage'
        ,request: {
    ...
    table.on('tool(layTable)', function(obj){
        ...
        console.log(data.id);
        if(obj.event === 'email'){
            /*layer.msg('ID：'+ data.id + ' 的发送邮件操作');*/
        	let url = '/recruit/candidate/candidateEmail?id='+data.id;
        	console.log(url);
            x_admin_show('发送邮件',url);
            ...
            console.log(data.id);
            let url = '/recruit/candidate/candidateUpdate?id='+data.id;
            console.log(url);
            x_admin_show('编辑',url);
...
function member_del(id){
    let url = "/recruit/candidate/candidateDeleteAjax?id="+id;
...
function sendSome() {
	...
	console.log(arr);
	$.post("/recruit/candidate/candidateEmailSomeAjax",
			{"ids":arr},function(){}
    )
...
function initTable(name,sex) {
	layui.use('table', function(){
	   ...
	   table.render({
	        ...
	        ,page:true
	        //异步请求响应
	        ,url: '/recruit/candidate/selectLikeAjax?name='+name+"&sex="+sex
	        ,request: {
	            pageName: 'pageNum' //页码的参数名称，默认：page
	            ,limitName: 'pageSize' //每页数据量的参数名，默认：limit
	        }
