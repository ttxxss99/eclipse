...
layui.use('table', function(){
   ...
   table.render({
        ...
        ,cols: [[
            ...
            ,{field:'name', title:'节日名称', width:"10%"}
            ,{field:'time', title:'日期', width:"30%"}
            ,{field:'content', title:'祝福语', width:"30%"}
            ,{fixed: 'right', title:'操作', toolbar: '#barDemo', width:"30%"}
        ]]
        ,page:true
        //异步请求响应
        ,url: '/txs/activity/ajaxPageHolidayList'
        ,request: {
...
//            layer.msg('ID：'+ data.id + ' 的查看操作');
            let url = '/txs/activity/hoSendEmail'
            ...
            console.log(data.id);
            let url = '/txs/activity/hoUpdateOpt?id='+data.id;
            console.log(url);
            x_admin_show('编辑',url);
...
function initTable(name) {
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
            	 {type: 'checkbox', fixed: 'left'}
                 ,{field:'name', title:'节日名称', width:"30%"}
                 ,{field:'time', title:'日期', width:"30%"}
                 ,{field:'content', title:'祝福语', width:"30%"}
            ...
            ,page:true
            //异步请求响应
            ,url: '/txs/activity/holikeSearch?name='+name
            ,request: {
                pageName: 'pageNum' //页码的参数名称，默认：page
                ,limitName: 'pageSize' //每页数据量的参数名，默认：limit
            }
        ...
        table.on('tool(layTable)', function(obj){
            ...
            console.log(data.id);
            if(obj.event === 'detail'){
            	let url = '/txs/activity/hoSendEmail?id='+data.id
            	layer.msg()
                ...
                console.log(data.id);
                let url = '/txs/company/companyUpdate?id='+data.id;
                console.log(url);
                x_admin_show('编辑',url);
