...
layui.use('table', function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field:'name', title:'职位名称', width:"25%"}
            ,{field:'mark', title:'备注', width:"25%"}
            ,{field:'deptName', title:'所属部门', width:"25%"}
            ,{fixed: 'right', title:'操作', toolbar: '#barDemo', width:"25%"}
        ]]
        ,page:true
        //异步请求响应
        ,url: '/organization/position/positionListPage'
        ,request: {
    ...
    table.on('tool(layTable)', function(obj){
        ...
        if(obj.event === 'detail'){
            ...
            console.log(data.id);
            let url = '/organization/position/positionUpdate?id='+data.id;
            console.log(url);
            x_admin_show('编辑',url);
...
function member_del(id){
    let url = "/organization/position/deleteAPosition?Id="+id;
...
function initTable(name,deptId) {

    let url =  '/organization/position/positionListPage';
    console.log("=========");
    console.log([name,deptId]);
    if (name === undefined && deptId === undefined){
        console.log(1)
       // url =  '/organization/dept/deptListPage';
        ...
        console.log(3)
        url = '/organization/position/likeSearch?name='+name+"&deptId="+deptId;
    }
    console.log(url);
...
let zy = {
    ...
    selectInit:()=>{
        ...
        $.post(url,{},(json)=>{
            ...
            $("#selectedDeptName").append(totalText);
    ...
    bindLikeSearch:()=>{
        layui.use('form', function() {
            ...
            form.on('submit(sreach)', function(data){
                let [somePositionName,selectedDeptName] = [$("#somePositionName").val(),$("#selectedDeptName").val()];
              /*  console.log("------------")
                console.log(somePositionName);
                console.log(selectedDeptName);
                console.log(somePositionName=="" && selectedDeptName =="")*/
                initTable(somePositionName,selectedDeptName);
